...
{
    public function getAll(array $data = [], bool $paginate = false, int $perPage = 10): Collection|LengthAwarePaginator
    {
        $queryBuilder = Employee::query()
            ...
            ->when(
                ...
                fn (Builder $query, $companyName) =>
                $query->whereHas(
                    'company',
                    function (Builder $query) use ($companyName) {
            ...
            ->when(Arr::get($data, 'sort_by'), fn (Builder $query, $sortBy) => $query->sortBy($sortBy));
